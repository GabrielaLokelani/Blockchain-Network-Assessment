let Block,calculateBlockHash,MIEWCOIN_BLOCKCHAIN,Transaction,broadcast,responseLatestMsg;_d19‍.x([["default",()=>BlockChain]]);_d19‍.w("./block",[["default",["Block"],function(v){Block=v}],["calculateBlockHash",["calculateBlockHash"],function(v){calculateBlockHash=v}]]);_d19‍.w("../index",[["MIEWCOIN_BLOCKCHAIN",["MIEWCOIN_BLOCKCHAIN"],function(v){MIEWCOIN_BLOCKCHAIN=v}]]);_d19‍.w("./transaction",[["default",["Transaction"],function(v){Transaction=v}]]);_d19‍.w("./node",[["broadcast",["broadcast"],function(v){broadcast=v}],["responseLatestMsg",["responseLatestMsg"],function(v){responseLatestMsg=v}]]);
...
const CryptoJS = require('crypto-js');
    ...
    creationOfGenesisBlock() {
        return new (_d19‍.a("Block",Block))(0, [], "0000000000000000000000000000000000000000", 7/17/2023, '')
    }
    ...
    minePendingTransactions(miningRewardAddress) {
        ...
        let block = new (_d19‍.a("Block",Block))(newIndex, this.pendingTransactions, JSON.stringify(miningRewardAddress), Date.now(), latestBlock.blockHash);

        block.mineBlock(this.difficulty, block);
        console.log('Block was successfully mined!');

        // this.addBlock(block);
        _d19‍.a("MIEWCOIN_BLOCKCHAIN",MIEWCOIN_BLOCKCHAIN).addBlock(block);
    ...
    isChainValid() {
        for (let i = 1; i < this.chain.length; i++) {
            ...
            // testing purposes 
            _d19‍.g.console.log('current blockhash' + currentBlock.blockHash);
            _d19‍.g.console.log('calculated blockhash' + currentBlock.calculateBlockHash());
            // Check if all transactions in the block are valid.
            if (!currentBlock.checkTransactionsValidity()) {
                return false;
            }
            if (currentBlock.blockHash !== currentBlock.calculateBlockHash()) {
                _d19‍.g.console.error("hash not equal: " + JSON.stringify(currentBlock));
                return false;
            }
            if (currentBlock.previousBlockHash !== previousBlock.calculateBlockHash()) {
                _d19‍.g.console.error("previous hash is not right: " + JSON.stringify(currentBlock));
                return false;
            }
        }
        return true;
    ...
    isValidNewBlock(newBlock, previousBlock) {
        if (previousBlock.index + 1 !== newBlock.index) {
            ...
            return false;
        } else if (_d19‍.a("calculateBlockHash",calculateBlockHash)(newBlock) !== newBlock.blockHash) {
    ...
    addBlock(newBlock) {
        const latestBlock = this.getBlock(this.getHeight());
        _d19‍.g.console.log('is new block valid? ' + this.isValidNewBlock(newBlock, latestBlock))
    ...
    replaceChain(newBlocks) {
        if (isValidChain(newBlocks) && newBlocks.length > this.chain.length) {
            ...
            _d19‍.a("broadcast",broadcast)(_d19‍.a("responseLatestMsg",responseLatestMsg)());
        } else {
