let Block,calculateBlockHash,createDate,MIEWCOIN_BLOCKCHAIN,Transaction,broadcast,responseLatestMsg;_d8c‍.x([["default",()=>BlockChain]]);_d8c‍.w("./block",[["default",["Block"],function(v){Block=v}],["calculateBlockHash",["calculateBlockHash"],function(v){calculateBlockHash=v}],["createDate",["createDate"],function(v){createDate=v}]]);_d8c‍.w("../index",[["MIEWCOIN_BLOCKCHAIN",["MIEWCOIN_BLOCKCHAIN"],function(v){MIEWCOIN_BLOCKCHAIN=v}]]);_d8c‍.w("./transaction",[["default",["Transaction"],function(v){Transaction=v}]]);_d8c‍.w("./node",[["broadcast",["broadcast"],function(v){broadcast=v}],["responseLatestMsg",["responseLatestMsg"],function(v){responseLatestMsg=v}]]);
...
const CryptoJS = require('crypto-js');
    ...
    creationOfGenesisBlock() {
        return new (_d8c‍.a("Block",Block))(0, [], "0000000000000000000000000000000000000000", _d8c‍.a("createDate",createDate)(), '')
    }
    ...
    minePendingTransactions(miningRewardAddress) {
        ...
        let block = new (_d8c‍.a("Block",Block))(newIndex, this.pendingTransactions, miningRewardAddress, _d8c‍.a("createDate",createDate)(), latestBlock.blockHash);

        block.mineBlock(2, block);
        console.log('Block was successfully mined!');

        _d8c‍.a("MIEWCOIN_BLOCKCHAIN",MIEWCOIN_BLOCKCHAIN).addBlock(block);
        ...
        _d8c‍.g.console.log("total fees in this block:   " + totalFees);
        let totalReward = totalFees + this.miningReward;
        const minerTXN = new (_d8c‍.a("Transaction",Transaction))("0000000000000000000000000000000000000000", miningRewardAddress, totalReward, 0, _d8c‍.a("createDate",createDate)(), "mining reward!", "00000000000000000000000000000000000000000000000000");
...
// ***!!! TURNED OFF FOR NOW WHILE TESTING OTHER ISSUE WITH SIGNING ***!!!
    ...
    isChainValid() {
        for (let i = 1; i < this.chain.length; i++) {
            ...
            // testing purposes 
            _d8c‍.g.console.log('current blockhash' + currentBlock.blockHash);
            _d8c‍.g.console.log('calculated blockhash' + currentBlock.calculateBlockHash());
            // Check if all transactions in the block are valid.
            if (!currentBlock.checkTransactionsValidity()) {
                return false;
            }
            if (currentBlock.blockHash !== currentBlock.calculateBlockHash()) {
                _d8c‍.g.console.error("hash not equal: " + JSON.stringify(currentBlock));
                return false;
            }
            if (currentBlock.previousBlockHash !== previousBlock.calculateBlockHash()) {
                _d8c‍.g.console.error("previous hash is not right: " + JSON.stringify(currentBlock));
                return false;
            }
        }
        return true;
    ...
    isValidNewBlock(newBlock, previousBlock) {
        if (previousBlock.index + 1 !== newBlock.index) {
            ...
            return false;
        } else if (_d8c‍.a("calculateBlockHash",calculateBlockHash)(newBlock) !== newBlock.blockHash) {
            return false;
        }
        return true;
    }
    ...
    addBlock(newBlock) {
        const latestBlock = this.getBlock(this.getHeight());
        _d8c‍.g.console.log('is new block valid? ' + this.isValidNewBlock(newBlock, latestBlock))
    ...
    replaceChain(newBlocks) {
        if (this.isValidChain(newBlocks) && newBlocks.length > this.chain.length) {
            ...
            _d8c‍.a("broadcast",broadcast)(_d8c‍.a("responseLatestMsg",responseLatestMsg)());
        } else {
