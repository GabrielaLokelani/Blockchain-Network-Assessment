let Block,calculateBlockHash,createDate,mineNewBlock,MIEWCOIN_BLOCKCHAIN,Transaction,broadcast,responseLatestMsg,faucetAddress;_16d‍.x([["default",()=>BlockChain]]);_16d‍.w("./block",[["default",["Block"],function(v){Block=v}],["calculateBlockHash",["calculateBlockHash"],function(v){calculateBlockHash=v}],["createDate",["createDate"],function(v){createDate=v}],["mineNewBlock",["mineNewBlock"],function(v){mineNewBlock=v}]]);_16d‍.w("../index",[["MIEWCOIN_BLOCKCHAIN",["MIEWCOIN_BLOCKCHAIN"],function(v){MIEWCOIN_BLOCKCHAIN=v}]]);_16d‍.w("./transaction",[["default",["Transaction"],function(v){Transaction=v}]]);_16d‍.w("./node",[["broadcast",["broadcast"],function(v){broadcast=v}],["responseLatestMsg",["responseLatestMsg"],function(v){responseLatestMsg=v}]]);_16d‍.w("./faucetWallet",[["faucetAddress",["faucetAddress"],function(v){faucetAddress=v}]]);// IMPORT RELEVANT LIBRARIES, VARIABLES AND FUNCTIONS
...
// create the blockchain class that will hold the chain
    ...
    creationOfGenesisBlock() {
        let genesisBlock = new (_16d‍.a("Block",Block))(0, [{
    ...
    minePendingTransactions(miningRewardAddress) {
        ...
        let block = new (_16d‍.a("Block",Block))(newIndex, this.pendingTransactions, 2, miningRewardAddress, latestBlock.blockHash);

        let totalFees = 0;
        for (const txn of block.transactions) {
            totalFees =  math.add(totalFees, txn.fee);
            _16d‍.g.console.log("total fees: " + totalFees);
        }
        let totalReward = math.add(totalFees, this.miningReward);
        const minerTXN = new (_16d‍.a("Transaction",Transaction))("0000000000000000000000000000000000000000", miningRewardAddress, totalReward, 0, _16d‍.a("createDate",createDate)(), "coinbase tx", "00000000000000000000000000000000000000000000000000");
        ...
        _16d‍.a("MIEWCOIN_BLOCKCHAIN",MIEWCOIN_BLOCKCHAIN).addBlock(block);
    ...
    getMiningJob(miningRewardAddress) {
        ...
        let block = new (_16d‍.a("Block",Block))(newIndex, this.pendingTransactions, 5, miningRewardAddress, latestBlock.blockHash);
    ...
    mineBlockCandidate(blockDataHash) {
        if (blockDataHash != undefined) {
            ...
            block.nonce = 0;
            block.dateCreated = _16d‍.a("createDate",createDate)();
            block.blockHash = '';
            let minedBlock = _16d‍.a("mineNewBlock",mineNewBlock)(5, block);
            return minedBlock;
        } else {
    ...
    submitMinedBlock(blockHash, dateCreated, nonce, blockDataHash) {
        ...
        const latestBlock = this.getBlock(this.getHeight());
        _16d‍.g.console.log("Is the blockDataHash matching? ", blockDataHash === candidate.blockDataHash);
        if (candidate.index === latestBlock.index + 1) {
            let candidateBlockHash = _16d‍.a("calculateBlockHash",calculateBlockHash)(candidate.blockDataHash, dateCreated, nonce);
            if (candidateBlockHash === blockHash) {
                candidate.nonce = nonce;
                candidate.dateCreated = dateCreated;
                candidate.blockHash = blockHash
                _16d‍.a("MIEWCOIN_BLOCKCHAIN",MIEWCOIN_BLOCKCHAIN).addBlock(candidate);
                miningJobs.clear();
                this.pendingTransactions = [];
    ...
    getPendingBalanceOfAddress(address) {
        let balance = 0;
        _16d‍.g.console.log("this is the chain length: " + this.chain.length);
    ...
    allBalances() {
        // if (this.chain.length >= 7) {
            confirmedAddressBalance.forEach((values, key, map) => {
                _16d‍.g.console.log("this is the values and keys for the balance" , values, key, map)
                let address = key;
    ...
    getConfirmedTransactions() {
        ...
        if (this.chain.length >= 7) {
            for (const block of this.chain) {
                ...
                _16d‍.g.console.log("here is the transaction for getConfirmedTransactions: ", transaction);
                return transaction
                }       
            }
        }
    ...
    isChainValid() {
        for (let i = 1; i < this.chain.length; i++) {
            ...
            // testing purposes 
            _16d‍.g.console.log('current blockhash' + currentBlock.blockHash);
            _16d‍.g.console.log('calculated blockhash' + currentBlock.calculateBlockHash(currentBlock.blockDataHash, currentBlock.dateCreated, currentBlock.nonce));
            ...
            if (currentBlock.blockHash !== currentBlock.calculateBlockHash(currentBlock.blockDataHash, currentBlock.dateCreated, currentBlock.nonce)) {
                _16d‍.g.console.error("hash not equal: " + JSON.stringify(currentBlock));
                return false;
            }
            if (currentBlock.previousBlockHash !== previousBlock.calculateBlockHash(previousBlock.blockDataHash, previousBlock.dateCreated, previousBlock.nonce)) {
                _16d‍.g.console.error("previous hash is not right: " + JSON.stringify(currentBlock));
                return false;
            }
        }
        return true;
    ...
    isValidNewBlock(newBlock, previousBlock) {
        if (previousBlock.index + 1 !== newBlock.index) {
            ...
            return false;
        } else if (_16d‍.a("calculateBlockHash",calculateBlockHash)(newBlock.blockDataHash, newBlock.dateCreated, newBlock.nonce) !== newBlock.blockHash) {
            return false;
        }
        return true;
    }
    ...
    addBlock(newBlock) {
        const latestBlock = this.getBlock(this.getHeight());
        _16d‍.g.console.log('is new block valid? ' + this.isValidNewBlock(newBlock, latestBlock))
    ...
    replaceChain(newBlocks) {
        if (this.isValidChain(newBlocks) && newBlocks.length > this.chain.length) {
            ...
            _16d‍.a("broadcast",broadcast)(_16d‍.a("responseLatestMsg",responseLatestMsg)());
        } else {
