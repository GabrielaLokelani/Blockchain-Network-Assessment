let Block,calculateBlockHash,createDate,mineNewBlock,MIEWCOIN_BLOCKCHAIN,Transaction,broadcast,responseLatestMsg,faucetAddress;_8ee‍.x([["miningJobs",()=>miningJobs],["confirmedAddressBalance",()=>confirmedAddressBalance],["default",()=>BlockChain]]);_8ee‍.w("./block",[["default",["Block"],function(v){Block=v}],["calculateBlockHash",["calculateBlockHash"],function(v){calculateBlockHash=v}],["createDate",["createDate"],function(v){createDate=v}],["mineNewBlock",["mineNewBlock"],function(v){mineNewBlock=v}]]);_8ee‍.w("../index",[["MIEWCOIN_BLOCKCHAIN",["MIEWCOIN_BLOCKCHAIN"],function(v){MIEWCOIN_BLOCKCHAIN=v}]]);_8ee‍.w("./transaction",[["default",["Transaction"],function(v){Transaction=v}]]);_8ee‍.w("./node",[["broadcast",["broadcast"],function(v){broadcast=v}],["responseLatestMsg",["responseLatestMsg"],function(v){responseLatestMsg=v}]]);_8ee‍.w("./faucetWallet",[["faucetAddress",["faucetAddress"],function(v){faucetAddress=v}]]);// IMPORT RELEVANT LIBRARIES, VARIABLES AND FUNCTIONS
...
// create mining job mempool
       let miningJobs = new Map();;_8ee‍.j(["miningJobs"]);
       let confirmedAddressBalance = new Map();;_8ee‍.j(["confirmedAddressBalance"]);
const miningReward = 5000000;
_8ee‍.g.console.log("This is the mining reward: ", BigInt(miningReward));
let bigIntReward = BigInt(miningReward);
_8ee‍.g.console.log("This is the mining reward to number again: ", Number(bigIntReward));
...
// create the blockchain class that will hold the chain
    ...
    creationOfGenesisBlock() {
        let genesisBlock = new (_8ee‍.a("Block",Block))(0, [{
    ...
    minePendingTransactions(miningRewardAddress) {
        ...
        let block = new (_8ee‍.a("Block",Block))(newIndex, this.pendingTransactions, 2, miningRewardAddress, latestBlock.blockHash);

        let totalFees = 0;
        for (const txn of block.transactions) {
            totalFees =  math.add(totalFees, txn.fee);
            _8ee‍.g.console.log("total fees: " + totalFees);
        }
        let totalReward = math.add(totalFees, miningReward);
        const minerTXN = new (_8ee‍.a("Transaction",Transaction))("0000000000000000000000000000000000000000", miningRewardAddress, totalReward, 0, _8ee‍.a("createDate",createDate)(), "coinbase tx", "00000000000000000000000000000000000000000000000000");
        ...
        _8ee‍.a("MIEWCOIN_BLOCKCHAIN",MIEWCOIN_BLOCKCHAIN).addBlock(block);
    ...
    getMiningJob(miningRewardAddress) {
        ...
        let block = new (_8ee‍.a("Block",Block))(newIndex, this.pendingTransactions, 5, miningRewardAddress, latestBlock.blockHash);
    ...
    mineBlockCandidate(blockDataHash) {
        if (blockDataHash != undefined) {
            ...
            block.nonce = 0;
            block.dateCreated = _8ee‍.a("createDate",createDate)();
            block.blockHash = '';
            let minedBlock = _8ee‍.a("mineNewBlock",mineNewBlock)(5, block);
            return minedBlock;
        } else {
    ...
    submitMinedBlock(blockHash, dateCreated, nonce, blockDataHash) {
        ...
        const latestBlock = this.getBlock(this.getHeight());
        _8ee‍.g.console.log("Is the blockDataHash matching? ", blockDataHash === candidate.blockDataHash);
        if (candidate.index === latestBlock.index + 1) {
            let candidateBlockHash = _8ee‍.a("calculateBlockHash",calculateBlockHash)(candidate.blockDataHash, dateCreated, nonce);
            if (candidateBlockHash === blockHash) {
                candidate.nonce = nonce;
                candidate.dateCreated = dateCreated;
                candidate.blockHash = blockHash
                _8ee‍.a("MIEWCOIN_BLOCKCHAIN",MIEWCOIN_BLOCKCHAIN).addBlock(candidate);
                miningJobs.clear();
                this.pendingTransactions = [];
    ...
    getCurrentCumulativeDifficulty() {
        ...
        _8ee‍.g.console.log("This is the current cumulative difficulty: ", currentCumulativeDifficulty);
    ...
    isCurrentCumDiffLT(blockchainToValidate) {
        ...
        _8ee‍.g.console.log("This is the cumulative difficulty of our chain: ", currentCumulativeDifficulty);
        _8ee‍.g.console.log("This is the cumulative difficulty of the other chain: ", otherCurrentCumulativeDifficulty);
    ...
    isChainValid() {
        for (let i = 1; i < this.chain.length; i++) {
            ...
            // testing purposes 
            _8ee‍.g.console.log('current blockhash' + currentBlock.blockHash);
            _8ee‍.g.console.log('calculated blockhash' + currentBlock.calculateBlockHash(currentBlock.blockDataHash, currentBlock.dateCreated, currentBlock.nonce));
            ...
            if (currentBlock.blockHash !== currentBlock.calculateBlockHash(currentBlock.blockDataHash, currentBlock.dateCreated, currentBlock.nonce)) {
                _8ee‍.g.console.error("hash not equal: " + JSON.stringify(currentBlock));
                return false;
            }
            if (currentBlock.previousBlockHash !== previousBlock.calculateBlockHash(previousBlock.blockDataHash, previousBlock.dateCreated, previousBlock.nonce)) {
                _8ee‍.g.console.error("previous hash is not right: " + JSON.stringify(currentBlock));
                return false;
            }
        }
        return true;
    ...
    isValidNewBlock(newBlock, previousBlock) {
        ...
        if (previousBlock.index + 1 !== newBlock.index) {
            ...
            return false;
        } else if (_8ee‍.a("calculateBlockHash",calculateBlockHash)(newBlock.blockDataHash, newBlock.dateCreated, newBlock.nonce) !== newBlock.blockHash) {
    ...
    addBlock(newBlock) {
        const latestBlock = this.getBlock(this.getHeight());
        _8ee‍.g.console.log('is new block valid? ' + this.isValidNewBlock(newBlock, latestBlock))
    ...
    replaceChain(newBlocks) {
        if (this.isValidChain(newBlocks) && newBlocks.length > this.chain.length) {
            ...
            miningJobs.clear();
            _8ee‍.a("broadcast",broadcast)(_8ee‍.a("responseLatestMsg",responseLatestMsg)());
        } else {
