let Block,MIEWCOIN_BLOCKCHAIN,Transaction,broadcast,responseLatestMsg;_51f‍.x([["default",()=>BlockChain]]);_51f‍.w("./block",[["default",["Block"],function(v){Block=v}]]);_51f‍.w("../index",[["MIEWCOIN_BLOCKCHAIN",["MIEWCOIN_BLOCKCHAIN"],function(v){MIEWCOIN_BLOCKCHAIN=v}]]);_51f‍.w("./transaction",[["default",["Transaction"],function(v){Transaction=v}]]);_51f‍.w("./node",[["broadcast",["broadcast"],function(v){broadcast=v}],["responseLatestMsg",["responseLatestMsg"],function(v){responseLatestMsg=v}]]);
...
const CryptoJS = require('crypto-js');
    ...
    creationOfGenesisBlock() {
        return new (_51f‍.a("Block",Block))(0, [], "0000000000000000000000000000000000000000", 7/17/2023, '')
    }
    ...
    minePendingTransactions(miningRewardAddress) {
        ...
        let block = new (_51f‍.a("Block",Block))(newIndex, this.pendingTransactions, JSON.stringify(miningRewardAddress), Date.now(), latestBlock.blockHash);

        block.mineBlock(this.difficulty, block);
        console.log('Block was successfully mined!');

        // this.addBlock(block);
        _51f‍.a("MIEWCOIN_BLOCKCHAIN",MIEWCOIN_BLOCKCHAIN).addBlock(block);
    ...
    isChainValid() {
        for (let i = 1; i < this.chain.length; i++) {
            ...
            // testing purposes 
            _51f‍.g.console.log('current blockhash' + currentBlock.blockHash);
            _51f‍.g.console.log('calculated blockhash' + currentBlock.calculateBlockHash());
            // Check if all transactions in the block are valid.
            if (!currentBlock.checkTransactionsValidity()) {
                return false;
            }
            if (currentBlock.blockHash !== currentBlock.calculateBlockHash()) {
                _51f‍.g.console.error("hash not equal: " + JSON.stringify(currentBlock));
                return false;
            }
            if (currentBlock.previousBlockHash !== previousBlock.calculateBlockHash()) {
                _51f‍.g.console.error("previous hash is not right: " + JSON.stringify(currentBlock));
                return false;
            }
        }
        return true;
    ...
    addBlock(newBlock) {
        const latestBlock = this.getBlock(this.getHeight());
        _51f‍.g.console.log('is new block valid? ' + this.isValidNewBlock(newBlock, latestBlock))
    ...
    replaceChain(newBlocks) {
        if (isValidChain(newBlocks) && newBlocks.length > this.chain.length) {
            ...
            _51f‍.a("broadcast",broadcast)(_51f‍.a("responseLatestMsg",responseLatestMsg)());
        } else {
