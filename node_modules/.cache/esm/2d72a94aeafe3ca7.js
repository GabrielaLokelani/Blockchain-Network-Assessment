let Block,Transaction;_eaf‍.x([["default",()=>BlockChain]]);_eaf‍.w("./block",[["default",["Block"],function(v){Block=v}]]);_eaf‍.w("./transaction",[["default",["Transaction"],function(v){Transaction=v}]]);
...
const CryptoJS = require('crypto-js');
    ...
    isChainValid() {
        for (let i = 1; i < this.chain.length; i++) {
            ...
            // testing purposes 
            _eaf‍.g.console.log('current blockhash' + currentBlock.blockHash);
            _eaf‍.g.console.log('calculated blockhash' + currentBlock.calculateBlockHash());
            // Check if all transactions in the block are valid.
            if (!currentBlock.checkTransactionsValidity()) {
                return false;
            }
            if (currentBlock.blockHash !== currentBlock.calculateBlockHash()) {
                _eaf‍.g.console.error("hash not equal: " + JSON.stringify(currentBlock));
                return false;
            }
            if (currentBlock.previousBlockHash !== previousBlock.calculateBlockHash()) {
                _eaf‍.g.console.error("previous hash is not right: " + JSON.stringify(currentBlock));
                return false;
            }
        }
        return true;
    ...
    addBlock(newBlock) {
        const latestBlock = this.getBlock(this.getHeight());
        _eaf‍.g.console.log('is new block valid? ' + this.isValidNewBlock(newBlock, latestBlock))
